...
class BaseAudioConfig(Coqpit):
    """Base configuration class to define audio processing parameters.
    
    This class provides configuration parameters used to initialize the ```TTS.utils.audio.AudioProcessor```.
    It contains parameters for STFT computation, audio processing, silence trimming, normalization,
    and mel-spectrogram generation.
    ...
    def check_values(
        ...
        check_argument("trim_db", c, restricted=True, min_val=0, max_val=100)
        
        # validate pitch parameters
        check_argument("pitch_fmin", c, restricted=True, min_val=0.0, max_val=1000.0)
        check_argument("pitch_fmax", c, restricted=True, min_val=1.0, max_val=2000.0)
        
        # validate sampling parameters
        check_argument("resample", c, restricted=True)
        check_argument("do_sound_norm", c, restricted=True)
        check_argument("do_rms_norm", c, restricted=True)
        if c["do_rms_norm"] and c["db_level"] is not None:
            check_argument("db_level", c, restricted=True, min_val=-99, max_val=0)
...
class BaseDatasetConfig(Coqpit):
    ...
    def check_values(
        ...
        """Check config fields for validity.
        
        Validates required fields and their values, ensuring they meet the expected format and constraints.
        """
        c = asdict(self)
        ...
        check_argument("phonemizer", c, restricted=False)
        
        # Validate that ignored_speakers is a list if provided
        if c["ignored_speakers"] is not None:
            if not isinstance(c["ignored_speakers"], list):
                raise ValueError("ignored_speakers must be a list of strings if provided")
